package shop.bejing.controller;
...
import shop.bejing.dto.user.JoinReqDto;
import shop.bejing.dto.user.LoginReqDto;
import shop.bejing.handler.ex.CustomException;
import shop.bejing.model.user.User;
import shop.bejing.model.user.UserRepository;
import shop.bejing.service.UserService;
...
public class UserController {
    ...
    /*
     * POST 요청과 PUT 요청시에만 BODY 데이터가 있다.
     * 해당 BODY 데이터는 컨트롤러 메서드의 매개변수에 주입된다.(DS)
     * 스프링은 x-www-form-urlencoded 가 기본 파싱 전략
     * key= value&key=value (form 태그의 기본 전송 전략)
     * 스프링은 매게변수에서 두가지 방식으로 데이터를 받는다.
     * 1. 그냥 변수 2. DTO(Object)
     * 주의 : key 이름과 변수 이름이 동일해야한다.
     */

    @PostMapping("/join")
    public String join(JoinReqDto joinReqDto) { // DTO로 받는 것이 좋다.
        // 1. POST, PUT일 때만 유효성 검사 (이것보다 우선되는 것이 인증 검사이다)
        // 유효성 검사 : POST 랑 PUT 만 바디 데이터가 있다. (리소스를 전달할때,무엇을 주어야할때 유효성 검사가 필요하다.)
